...
* fixed bug with parsing when last variable is a string one, thanks Mikko R枚nkk枚
...
mark `touse' `if' `in'
    ...
    global mainfile = "/Users/liding/temp/a.txt"
    *默认覆盖原输出
    local opt ""
...
*------------------------ display options -------------------------
    ...
    *默认不显示输出内容
    global show = cond("`show'"~="","`show'","none") 

*------------------------ n options -------------------------   
    
    global do_n = 0
    *单元格为行百分比则列样本数，单元格为列百分比则行样本数
    global n_pos ="col"
   * global n_pos =cond("`cells'"=="col","row","col")
   * local n_pos =cond("`cells'"=="col","row","col")
    global n_wt = ""
    global n_offset = "0"
    ...
    *李丁加的默认条件
    if "`npos'"=="" {
            if ("`cells'")=="col" global n_pos = "row"
                else global n_pos = "col"
            global do_n 1
    }
...
*===================== main routine =============================   
    ...
    *李丁加了一句
    if `nvars'==2 &"`cells'"== "col" tab  `varlist' if `touse',col chi
    if `nvars'==2 &"`cells'"== "row" tab  `varlist' if `touse',row chi 
...
di
di as text `"Table output written to: {browse $mainfile}"'
di
if ("$show"=="all" | "$show"=="output") type "$mainfile"
*view "`using'", asis
